...
export default {
  ...
  methods: {
    async getDeptList() {
      let list = await this.$api.dept.getDeptList(this.queryForm);
      this.deptList = list;
      let rowKeys = [];
    ...
    async getAllUserList() {
      this.userList = await this.$api.user.getAllUserList();
    },
    handleUser(val) {
      console.log("=>", val);
    ...
    async handleDel(_id) {
      this.action = "delete";
      await this.$api.dept.deptOperate({_id, action: this.action});
    ...
    handleSubmit() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          delete params.user;
          await this.$api.dept.deptOperate(params);
